...
module_vps_hopingboyz() {
  ...
  bash <(curl -s "$url")
  log "${GREEN}✔ VPS Manager executed.${NC}"
}
...
main_menu() {
  while true; do
    ...
    printf "  %b6%b) %bVPS Management by @Hopingboyz%b\n" "${YELLOW}" "${NC}" "${GREEN}" "${NC}"
    printf "  %b7%b) %bShow logs%b\n" "${YELLOW}" "${NC}" "${GREEN}" "${NC}"
    printf "  %b0%b) %bExit%b\n\n" "${YELLOW}" "${NC}" "${RED}" "${NC}"
    ...
    case "$choice" in
      ...
      7) ${SUDO} tail -n 50 /var/log/24-7.log 2>/dev/null || log "${YELLOW}No log found.${NC}" ;;
      0) log "${GREEN}Goodbye!${NC}"; exit 0 ;;
      *) log "${RED}Invalid choice${NC}" ;;
    esac
    read -rp $'\e[1;36mPress Enter to continue...\e[0m'
  done
}
...
clear
print_static_header
log "${YELLOW}Dependencies checked. Loading menu...${NC}"
sleep 0.5
main_menu
